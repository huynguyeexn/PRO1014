...
function getAllOrder(){
    $sql = "SELECT * FROM $dbname.`order` order BY created DESC;";
    return query($sql);
}

// Lấy đơn hàng theo vị trí
function getOrderByOffset($limit, $offset){
    $sql = "SELECT * FROM $dbname.`order` LIMIT $limit OFFSET $offset;";
    return query($sql);
}

// Lấy đơn hàng theo id
function getOrderById($id){
    $sql = "SELECT * FROM $dbname.`order` WHERE id = $id;";
    return queryOne($sql);
}

// Lấy các đơn hàng của một khách hàng
function getOrderByUser($userid){
    $sql = "SELECT * FROM $dbname.`order` WHERE user_id = $userid  order BY created desc;";
    return query($sql);
}
...
function deleteOrder($id){
    $sql = "DELETE FROM $dbname.`order` WHERE id = $id";
    return execute($sql);
}
?>